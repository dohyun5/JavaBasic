...
public class Application05 {
	...
	public static void main(String[] args) {
		// 클래스 + 배열 => Student[] ary = new Student[10] -> 10개의 공간에 학생 객체를 저장
		
		// int + 배열 => int[] ary = new int[10]; -> 10개의 공간에 점수
	
		//1. 학생의 수  | 2. 학생들 정보 입력 | 3. 학생들의 총점, 평균 | 4. 종료
		...
		while(true) {
			...
			if(selectNo.equals("1")) {
				...
				for(int i=0; i<stdAry.length;i++) {
					//i번째 배열에 학생 클래스를 활용한 객체생성
					stdAry[i] = new Student();
					//학생들의 정보를 입력하는 공간
					//객체를 생성하고 객체의 정보를 저장한다음 -> 배열에 보관
					System.out.println("학생 이름>");
					String name = sc.nextLine();
					stdAry[i].name = name;  //이 한문장이 중요!
					//s1이라고 가정
					//i = 0
					//stdAry[0] = s1
					//stdAry[0].name = s1.name
				...
				for(int i=0;i<stdAry.length;i++) {
					...
					//stdAry[i].k + stdAry[i].m + stdAry[i].e ->총점
